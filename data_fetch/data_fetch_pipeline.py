...
def clean_task():
    def write_to_csv(text):
        with open(
            "/Users/ganeshs/Desktop/Code/CS5830/project/cleaned_reviews.csv", "a"
        ) as file:
            file.write(text)
    ...
    with beam.Pipeline(options=PipelineOptions()) as pipeline:
        pcollection = (
            ...
            >> beam.Create(
                read_reviews_csv(
                    "/Users/ganeshs/Desktop/Code/CS5830/project/trustpilot_reviews.csv"
                )
            )
            | "Clean text" >> beam.Map(clean_reviews)
            | "Format to csv" >> beam.Map(lambda x: f"{x['content']},{x['rating']}\n")
            | "Write output" >> beam.Map(write_to_csv)
        )
...
def data_fetch_task():
    def write_to_csv(text):
        with open(
            "/Users/ganeshs/Desktop/Code/CS5830/project/trustpilot_reviews.csv", "a"
        ) as file:
            file.write(text)
    ...
    with beam.Pipeline(options=PipelineOptions()) as pipeline:
        pcollection = (
            ...
            >> beam.Create(
                read_text_file(
                    "/Users/ganeshs/Desktop/Code/CS5830/project/list_of_companies.txt"
                )
            )
...
with DAG(
    ...
    t1 = PythonOperator(
        task_id="scrape_data",
        python_callable=data_fetch_task,
